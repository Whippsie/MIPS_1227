...
beq $s0, $t0, done
addi $s0, $0, 1
# Sinon on verifie le nombre de repetition 
beq $s0, $t0, once
addi $s0, $s0, 1
beq $s0, $t0, twice
addi $s0, $s0, 1
...
lb $t2, 0($a1)
sb $t2, 0($a2)
#addi $a2, $a2, 1 -- fonctionne, mais comment dire ensuite qu'on veut l'element a l'adresse 1001 par exemple sans repasser par le load byte
j done

twice:
lb $t3, 0($a1)
sb $t3, 1($a2)
# On doit relooper, on repart à once
j once
